...
class SAM2SemiAutoBboxDetectionStage(
    PipelineStage[SAM2SemiAutoBboxDetectionRuntimeConfig]
):
    """
    ...
    def __init__(
        ...
        name: str,
        order: int,
        runtime_cfg: SAM2SemiAutoBboxDetectionRuntimeConfig,
        dynamic_runtime_cfg: (
            dict[str, SAM2SemiAutoBboxDetectionRuntimeConfig] | None
        ) = None,
    ...
    def forward(
        ...
        self.det_inferencer.model.to(device)
...
def _batch_infer_bboxes(
    ...
    for i in range(batch_size):
        bboxes = det_results[i].pred_instances.bboxes.float()
        scores = det_results[i].pred_instances.scores.float()
