...
def _initialize_graph(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        ...
        for pair_idx, (view_i, view_j) in tqdm(
            ...
            n_cheiral, E, R, t, inliers_mask = pose_futures[pair_idx].result()
            # recoverPose returns how many correspondences its chosen (R, t)
            # places in front of both cameras. Kept as a fraction so it is
            # comparable between pairs with different correspondence counts.
            cheirality_ratio = n_cheiral / max(len(undistorted_points1), 1)
...
def _warn_on_weak_tree_edges(graph: nx.DiGraph, mst: nx.Graph) -> None:
    ...
    inherited by every view downstream of it. A pair whose chosen (R, t) puts
    most of its correspondences behind the cameras is the clearest cheap sign
    of such an edge.
    ...
    """
    weak = []
    for view_i, view_j in mst.edges:
        if not graph.has_edge(view_i, view_j):
            continue
        ratio = graph.edges[view_i, view_j].get("cheirality_ratio")
        if ratio is not None and ratio < MIN_TREE_EDGE_CHEIRALITY_RATIO:
            weak.append((view_i, view_j, ratio))

    if weak:
        detail = ", ".join(f"{i}-{j} ({r:.0%})" for i, j, r in sorted(weak))
        warnings.warn(
            f"{len(weak)} spanning-tree edge(s) place most correspondences "
            f"behind the cameras: {detail}. Absolute poses are chained along "
            f"these edges, so the views beyond them are likely misplaced."
        )
