...
def pair_sampling_features(
    ...
    share a sensor size. ``w_uv`` is split across the four image axes so that
    the image-plane block does not outweigh the single time axis purely because
    it is spread over more dimensions.
...
class KeypointsPairsFpsSamplingRuntimeConfig:
...
class KeypointsPairsFpsSamplingStage(
    PipelineStage[KeypointsPairsFpsSamplingRuntimeConfig]
):
    """
    Samples the calibration point pairs with image-plane and time coverage.

    Drop-in alternative to :class:`KeypointsPairsSamplingStage`, producing the
    same :class:`CalibrationPointsAnnotations`. Off-frame and non-finite
    keypoints are rejected, then farthest-point sampling over
    ``[u_i, v_i, u_j, v_j, t]`` picks each pair's correspondences, so they span
    both image planes and the sequence duration instead of being an
    unstructured uniform draw.

    The random sampler branch keeps the same rejection filter, which isolates
    the filter from the sampler in the ablation.
    """
    ...
    def __init__(
        ...
        name: str,
        order: int,
        runtime_cfg: KeypointsPairsFpsSamplingRuntimeConfig,
        dynamic_runtime_cfg: dict[str, KeypointsPairsFpsSamplingRuntimeConfig]
        | None = None,
    ):
        super().__init__(
            name=name,
    ...
    def forward(
        ...
        # An off-frame or non-finite keypoint is exactly the kind of extreme
        # that would dominate the FPS selection, so it is dropped before
        # sampling.
        observations_mask = valid_observations_mask(kps_xy, resolutions_hw)
        ...
        for start in tqdm(
            ...
            if runtime_cfg.sampler == "random":
                picked_per_pair.extend(
                    self._random_pairs(candidates_mask, budget, generator)
                )
                continue
    ...
    def _random_pairs(
        ...
        """Uniform draw over the same candidate set the FPS branch sees.

        Args:
            candidates_mask: Per-pair candidates, (n_pairs, n_flat) bool.
            budget: Maximum correspondences per pair.
            generator: Seeded generator.

        Returns:
            One long tensor of flat candidate indices per pair.
        """
        picked = []

        for pair_mask in candidates_mask:
            valid_pick_indices = torch.where(pair_mask)[0]
            order = torch.randperm(
                valid_pick_indices.shape[0],
                generator=generator,
                device=valid_pick_indices.device,
            )[:budget]
            picked.append(valid_pick_indices[order])
